...
from app.ml.classifier import new_classifier
...
async def _run(record: ScanRecord, ws_manager) -> None:
    ...
    try:
        ...
        shadow_count = sum(
            1 for f in findings if f.is_shadow
        )
        ...
        await _emit(
            ...
            "complete",
            {
                ...
                "message": (
                    f"Scan complete — "
                    f"{len(findings)} services, "
                    f"{shadow_count} shadow detected"
                ),
            },
        )
...
async def _nmap_scan(
    ...
    await _emit(
        ...
        sid,
        "log",
        f"Nmap: starting {record.scan_type} scan with args: {args}",
    )
    ...
    try:
        ...
        await _emit(
            ...
            sid,
            "log",
            f"Nmap command: {nm.command_line()}",
        )
...
async def _amass_enum(
    ...
    await _emit(
        ...
        sid,
        "log",
        f"Amass: passive enumeration for {target}",
    )
    ...
    try:
        ...
        await _emit(
            ...
            sid,
            "log",
            f"Amass: found {len(assets)} assets",
        )
...
def _build_summary(
    ...
    sev = SeverityBreakdown(
        ...
        high=sum(
            1
            for f in findings
            if 60 <= f.risk_score < 80
        ),
        medium=sum(
            1
            for f in findings
            if 40 <= f.risk_score < 60
        ),
        low=sum(
            1
            for f in findings
